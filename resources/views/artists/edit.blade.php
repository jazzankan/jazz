                ...
                <form method="post" action="{{ route('artists.update',$artist->id) }}">
                    ...
                    <div class="pl-2">
                        ...
                        <div>
                            ...
                            <input type="text"
                                   class="max-w-lg w-full mt-2 mb-6 px-4 py-2 border rounded-lg text-gray-700 focus:outline-none focus:border-green-500"
                                   value="{{ $artist->memberof }}" name="memberof"/>
                        </div>
                        <div>
                            <label for="comment">Kommentar:</label><br>
                            <input type="text"
                                   class="max-w-lg w-full mt-2 mb-6 px-4 py-2 border rounded-lg text-gray-700 focus:outline-none focus:border-green-500"
                                   value="{{ $artist->comment }}" name="comment"/>
                        </div>
                        <div>
                            <label for="note">Intern anteckning:</label><br>
                            <input type="text"
                                   class="max-w-lg w-full mt-2 mb-6 px-4 py-2 border rounded-lg text-gray-700 focus:outline-none focus:border-green-500"
                                   value="{{ $artist->note }}" name="note"/>
                        </div>
                        <div>
                            <input type="checkbox" class="custom-control-input" id="delete" name="delete" value="delete"> Ta bort artisten.
                        </div>
